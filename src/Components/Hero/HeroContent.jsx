import "../../Styles/Hero/HeroContent.css"

function HeroContent(props) {
    const { img, poster, label, sublabel, likes, cheers, lengthSeconds } = props.data;
    return (

        //src={img} nelicencované fotky

        <div className="current-hero-label">
            <img className="hero-image" src="src\Images\placeholder.png" /> 
            <div className="current-container">
                <img className="hero-poster" src="src\Images\placeholder.png" />
                <div className="current-hero-label-info">
                    <svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" className="ipc-icon ipc-icon--play-circle-outline-large-inline sc-809ef538-5 gLhmTs slide-caption-play-icon" viewBox="0 0 24 24" fill="currentColor" role="presentation"><path d="M10.803 15.932l4.688-3.513a.498.498 0 0 0 0-.803l-4.688-3.514a.502.502 0 0 0-.803.402v7.026c0 .412.472.653.803.402z"></path><path d="M12 24C5.373 24 0 18.627 0 12S5.373 0 12 0s12 5.373 12 12-5.373 12-12 12zm0-1c6.075 0 11-4.925 11-11S18.075 1 12 1 1 5.925 1 12s4.925 11 11 11z"></path></svg>
                    <span className="label-text-area">
                        <h1>{label}</h1>
                        <h2>{sublabel}</h2>
                        <div className="hero-label-interactions">
                            <span>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" className="ipc-icon ipc-icon--thumb-up ipc-reaction-summary__likes-icon" viewBox="0 0 24 24" fill="currentColor" role="presentation"><path d="M13.12 2.06c.58-.59 1.52-.59 2.11-.01.36.36.51.87.41 1.37L14.69 8h5.65c2.15 0 3.6 2.2 2.76 4.18l-3.26 7.61C19.52 20.52 18.8 21 18 21H9c-1.1 0-2-.9-2-2V9.01c0-.53.21-1.04.58-1.41l5.54-5.54zM9.293 8.707A1 1 0 0 0 9 9.414V18a1 1 0 0 0 1 1h7.332a1 1 0 0 0 .924-.617c1.663-4.014 2.527-6.142 2.594-6.383.07-.253.12-.587.15-1v-.002A1 1 0 0 0 20 10h-8l1.34-5.34-4.047 4.047zM3 21c1.1 0 2-.9 2-2v-8c0-1.1-.9-2-2-2s-2 .9-2 2v8c0 1.1.9 2 2 2z"></path></svg>
                                {likes}
                            </span>
                            <span>
                                <svg viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" className="iconify iconify--twemoji" preserveAspectRatio="xMidYMid meet" fill="#000000"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"><path fill="#B78B60" d="M32.302 24.347c-.695-1.01-.307-2.47-.48-4.082c-.178-2.63-1.308-5.178-3.5-7.216l-7.466-6.942s-1.471-1.369-2.841.103c-1.368 1.471.104 2.84.104 2.84l3.154 2.934l2.734 2.542s-.685.736-3.711-2.078l-10.22-9.506s-1.473-1.368-2.842.104c-1.368 1.471.103 2.84.103 2.84l9.664 8.989c-.021-.02-.731.692-.744.68L5.917 5.938s-1.472-1.369-2.841.103c-1.369 1.472.103 2.84.103 2.84L13.52 18.5c.012.012-.654.764-.634.783l-8.92-8.298s-1.472-1.369-2.841.103c-1.369 1.472.103 2.841.103 2.841l9.484 8.82c.087.081-.5.908-.391 1.009l-6.834-6.356s-1.472-1.369-2.841.104c-1.369 1.472.103 2.841.103 2.841L11.896 30.71c1.861 1.731 3.772 2.607 6.076 2.928c.469.065 1.069.065 1.315.096c.777.098 1.459.374 2.372.934c1.175.72 2.938 1.02 3.951-.063l3.454-3.695l3.189-3.412c1.012-1.082.831-2.016.049-3.151z"></path><path d="M1.956 35.026a.999.999 0 0 1-.707-1.707L4.8 29.77a.999.999 0 1 1 1.414 1.414l-3.551 3.55a.996.996 0 0 1-.707.292zm6.746.922a.999.999 0 0 1-.944-1.33l.971-2.773a1 1 0 1 1 1.887.66l-.971 2.773a1 1 0 0 1-.943.67zm-7.667-7.667a1.001 1.001 0 0 1-.333-1.943l2.495-.881a1 1 0 1 1 .666 1.886l-2.495.881a1.001 1.001 0 0 1-.333.057zm29.46-21.767a1 1 0 0 1-.707-1.708l3.552-3.55a1 1 0 0 1 1.414 1.415l-3.552 3.55a.993.993 0 0 1-.707.293zm-4.164-1.697a1 1 0 0 1-.944-1.331l.97-2.773a1 1 0 0 1 1.888.66l-.97 2.773a1 1 0 0 1-.944.671zm6.143 5.774a1.001 1.001 0 0 1-.333-1.943l2.494-.881a1 1 0 1 1 .666 1.886l-2.494.881a1.001 1.001 0 0 1-.333.057z" fill="#FA743E"></path><path fill="#D5AB88" d="M35.39 23.822c-.661-1.032-.224-2.479-.342-4.096c-.09-2.634-1.133-5.219-3.255-7.33l-7.228-7.189s-1.424-1.417-2.843.008c-1.417 1.424.008 2.842.008 2.842l3.054 3.039l2.646 2.632s-.71.712-3.639-2.202c-2.931-2.915-9.894-9.845-9.894-9.845s-1.425-1.417-2.843.008c-1.418 1.424.007 2.841.007 2.841l9.356 9.31c-.02-.02-.754.667-.767.654L9.64 4.534s-1.425-1.418-2.843.007c-1.417 1.425.007 2.842.007 2.842l10.011 9.962c.012.012-.68.741-.66.761L7.52 9.513s-1.425-1.417-2.843.008s.007 2.843.007 2.843l9.181 9.135c.084.083-.53.891-.425.996l-6.616-6.583s-1.425-1.417-2.843.008s.007 2.843.007 2.843l10.79 10.732c1.802 1.793 3.682 2.732 5.974 3.131c.467.081 1.067.101 1.311.14c.773.124 1.445.423 2.34 1.014c1.15.759 2.902 1.118 3.951.07l3.577-3.576l3.302-3.302c1.049-1.05.9-1.99.157-3.15z"></path></g></svg>
                                {cheers}
                            </span>
                        </div>
                    </span>
                </div>
            </div>
            <span className="hero-video-length-span">{Math.floor(lengthSeconds / 60) + ":" + (lengthSeconds % 60)}</span>
        </div>

    )
}

export default HeroContent